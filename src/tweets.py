...
def plot_tweet_relations(tweet: Tweet) -> None:
    tree = get_tweet_graph(tweet)

    pos = networkx.spring_layout(tree)

    plt.figure(figsize=(16, 16))
    plt.title(f"{tweet.tweet_type}: '{tweet.content}'")
    ...
    networkx.draw_networkx_nodes(
        tree, pos,
        nodelist=[tweet.unique_id],
        node_color='g' if tweet.tweet_type is TweetType.TRUE else 'r',
        node_size=500,
        alpha=0.8
    )
...
def get_tweet_graph(tweet: Tweet) -> Graph:
    g = Graph()

    for current_tweet in [tweet] + tweet.all_children:
